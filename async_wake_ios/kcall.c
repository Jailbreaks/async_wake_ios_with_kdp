...
uint64_t kcall(uint64_t fptr, uint32_t argc, ...) {
  ...
  if (argc == 0) {
    printf("this primitive won't work without any arguments; pass at least one non-null argument\n");
    return 0;
  }
  
  if (args[0] == 0) {
    printf("this primitive won't work with a null first argument, sorry!\n");
    return 0;
  }
